...
public class OrderOtherControllerTest {
    ...
    public void testQueryOrders() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.post("/api/v1/orderOtherService/orderOther/query").contentType(MediaType.APPLICATION_JSON).content(requestJson))
                .andExpect(MockMvcResultMatchers.status().isOk())
    ...
    public void testQueryOrdersForRefresh() throws Exception {
        ...
        String requestJson = JSONObject.toJSONString(qi);
        mockMvc.perform(MockMvcRequestBuilders.post("/api/v1/orderOtherService/orderOther/refresh").contentType(MediaType.APPLICATION_JSON).content(requestJson))
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andExpect(MockMvcResultMatchers.jsonPath("$.status").value(response.getStatus()))
                .andExpect(MockMvcResultMatchers.jsonPath("$.msg").value(response.getMsg()));
    }
