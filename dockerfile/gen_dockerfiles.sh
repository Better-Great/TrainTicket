...
RUN echo '#!/bin/sh' > /entrypoint.sh && \\
    ...
    echo 'mkdir -p "\${PROJECT_ROOT}/properties" "\${PROJECT_ROOT}/${name}/src/main/resources"' >> /entrypoint.sh && \\
    echo 'cp "\$PROP_FILE" "\${PROJECT_ROOT}/properties/\${ENVIRONMENT}.application.ini"' >> /entrypoint.sh && \\
    echo 'cp "\${TEMPLATE_DIR}/application.properties.ini" "\${PROJECT_ROOT}/${name}/"' >> /entrypoint.sh && \\
    echo 'java -jar "\${JAR_DIR}/\${TOKEN_JAR_NAME}" "\${ENVIRONMENT}" "\${PROJECT_ROOT}"' >> /entrypoint.sh && \\
    echo 'if [ ! -f "\${PROJECT_ROOT}/${name}/src/main/resources/application.properties" ]; then echo "Error: Token replacement failed"; exit 1; fi' >> /entrypoint.sh && \\
    echo 'cp "\${PROJECT_ROOT}/${name}/src/main/resources/application.properties" "\${CONFIG_DIR}/application-external.properties"' >> /entrypoint.sh && \\
    ...
    echo '  --server.port="\${PORT:-${port}}" \\' >> /entrypoint.sh && \\
    echo '  --logging.file.name=\${LOGS_DIR}/${name}.log \\' >> /entrypoint.sh && \\
