...
import type { SecurityConfigItem } from '@/api/types'
import { useAdminStore } from '@/stores/admin'

const admin = useAdminStore()
const router = useRouter()

const configs = ref<SecurityConfigItem[]>([])
const loading = ref(false)
const busyId = ref('')
const error = ref('')
const ok = ref('')
const checkAccountId = ref('4d2a46c7-71ce-4cf1-b5bb-b68406a1fd6a')
...
async function refresh() {
  ...
  error.value = ''
  try {
    configs.value = (await listSecurityConfigs()).data ?? []
  } catch (e) {
    error.value = e instanceof Error ? e.message : 'Failed to load security configs'
  } finally {
    loading.value = false
  }
}
...
async function add() {
  ...
  ok.value = ''
  try {
    const res = await createSecurityConfig({ ...form })
...
function startEdit(c: SecurityConfigItem) {
  Object.assign(edit, c)
}
...
async function saveEdit() {
  ...
  ok.value = ''
  try {
    const res = await updateSecurityConfig({ ...edit })
    if (res.status !== 1) throw new Error(res.msg ?? 'Update failed')
    ok.value = 'Config updated'
    edit.id = ''
    await refresh()
  } catch (e) {
...
async function remove(id: string) {
  ...
  ok.value = ''
  try {
    const res = await deleteSecurityConfig(id)
...
async function runCheck() {
  ...
  checkResult.value = ''
  try {
    const res = await checkSecurity(checkAccountId.value.trim())
    const data = res.data
    checkResult.value = data.message ?? data.msg ?? (data.status ? 'Passed' : 'Failed')
  } catch (e) {
  ...
  <section class="page">
    <header class="head">
      <div>
        ...
        <h1>Security</h1>
        <p class="lede">
          Anti-scalping configs via <code>securityservice/securityConfigs</code> ·
    ...
    <h2>Account check</h2>
    <form class="check" @submit.prevent="runCheck">
      <input v-model="checkAccountId" aria-label="Account ID" required />
      <button type="submit">Check</button>
    </form>
    <p v-if="checkResult" class="ok" role="status">{{ checkResult }}</p>

    <h2>Configs</h2>
    <ul class="list" aria-live="polite">
      <li v-for="c in configs" :key="c.id" class="row">
        ...
        <template v-else>
          <div class="row-view">
            <div>
              ...
              <p class="meta">{{ c.value }} · {{ c.description }}</p>
            </div>
            <div class="actions">
      ...
      <li v-if="!configs.length && !loading" class="empty">No security configs.</li>
    </ul>
